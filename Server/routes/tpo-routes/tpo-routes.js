...
//set
router.post("/tpo/newRecord/:model", tpoUpload.single("Proof"), async (req, res) => {
    try {
...
//reset
router.post('/tpo/editRecord/:model', tpoUpload.single('Proof'), async (req, res) => {
    const model = req.params.model
    const data = JSON.parse(JSON.stringify(req.body));
    let SendData = null;
    const { id } = data
...
//remove
router.post('/tpo/deleteRecord', async (req, res) => {
    ...
    try {
        ...
        const link = path.join(__dirname, `../../uploads/tpo-uploads/${Filename}`);
