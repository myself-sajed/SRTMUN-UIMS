    ...
    if (reportType === 'CAS') {
        if (level !== 'stage4') {
            eligProofs = (stageObj?.[level]?.inputData?.map((inputData) => {
                return eligData?.[inputData.name]?.[0]?.filename && path.join(__dirname, `../uploads/faculty-uploads/${reportType}-uploads/${eligData?.[inputData.name]?.[0]?.filename}`)
        ...
        if (level === 'stage4') {
            eligProofs = stageObj?.[level]?.inputData?.map((inputData, index) => {
                return (index === 2 && eligData['supervisor'] === 'Main Supervisor') ? null :
                    eligData?.[inputData.name]?.[0]?.filename && path.join(__dirname, `../uploads/faculty-uploads/${reportType}-uploads/${eligData?.[inputData.name]?.[0]?.filename}`)
            }).filter(item => item !== undefined)
        }
    }
    ...
    casArray.forEach((casItem) => {
        ...
        proofs = filteredAcademicData.map((modelItem) => {
            return casItem.academicData?.['researchPaper'].scoreMap?.[modelItem._id]?.refreedIFProof &&
                path.join(__dirname, `../uploads/faculty-uploads/${reportType}-uploads/${casItem.academicData?.['researchPaper'].scoreMap?.[modelItem._id]?.refreedIFProof?.file?.[0].filename}`)
        }).filter(item => item !== undefined)


        // director proof
        if (casItem?.teachingData?.uploadedAttendance) {
            dirProof = path.join(__dirname, `../uploads/faculty-uploads/${reportType}-uploads/${casItem?.teachingData?.uploadedAttendance?.file[0]?.filename}`)
        }

        // activity proofs
        if (casItem?.teachingData?.checkBoxSelected?.length > 1) {
            actProof = casItem?.teachingData?.checkBoxSelected?.map((checkBoxId) => {
                return path.join(__dirname, `../uploads/faculty-uploads/${reportType}-uploads/${casItem?.teachingData?.uploadedFiles?.[`file-${checkBoxId}`]?.filename}`)
            })
        }
