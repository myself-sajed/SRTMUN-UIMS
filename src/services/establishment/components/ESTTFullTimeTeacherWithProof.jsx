...
import EsttFullTimeTeacher from '../pages/EsttFullTimeTeacher'


const ESTTFullTimeTeacherWithProof = ({ academicYear }) => {

    const [file, setFile] = useState(null)
    const [proof, setProof] = useState(null)

    const filter = { academicYear, userType: 'estt', proofType: 'ESTTFullTimeTeacher' }
    const { data, isLoading, refetch } = useQuery(`ESTTFullTimeTeacher-${academicYear}`, () => fetchSupportingDocuments(filter), { refetchOnWindowFocus: false })
    ...
    const submitProofFunction = () => {
        ...
        formData.append('proofType', 'ESTTFullTimeTeacher')
...
export default ESTTFullTimeTeacherWithProof
