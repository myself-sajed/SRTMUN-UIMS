...
function QualifiedExams({ id, setState, yearFilter, schoolName, Heading }) {
    ...
    let filter = yearFilter === '' && schoolName === '' ? null : yearFilter !== '' && schoolName === '' ? { Acadmic_year: yearFilter } : yearFilter === '' && schoolName !== '' ? { SchoolName: schoolName } : { Acadmic_year: yearFilter, SchoolName: schoolName }
    ...
    return (
        <AdminAcordinTable Heading={Heading} data={data?.data} SendReq={SendReq} proof='Upload_Proof' tableHead={tableHead} year='Acadmic_year' module='director' />
    );
}
