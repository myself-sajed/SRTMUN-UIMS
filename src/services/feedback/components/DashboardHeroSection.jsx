...
const DashboardHeroSection = ({ countData, isLoading, showDetails = false, year }) => {
    return <div className='grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-4'>
        <DashboardTile year={year} showDetails={showDetails} isLoading={isLoading} count={countData?.["StudentCount"]} title="Student" />
        <DashboardTile year={year} showDetails={showDetails} isLoading={isLoading} count={countData?.["TeacherCount"]} title="Teacher" />
        <DashboardTile year={year} showDetails={showDetails} isLoading={isLoading} count={countData?.["AlumniCount"]} title="Alumni" />
        <DashboardTile year={year} showDetails={showDetails} isLoading={isLoading} count={countData?.["ParentCount"]} title="Parent" />
        <DashboardTile year={year} showDetails={showDetails} isLoading={isLoading} count={countData?.["EmployerCount"]} title="Employer" />
    </div>
}


const DashboardTile = ({ title, count, isLoading, showDetails, year }) => {
    ...
    return (
        <div id="alert-additional-content-5" className="p-3 border border-blue-300 rounded-lg bg-blue-50 dark:border-blue-600 dark:bg-blue-800" role="alert">
            ...
            <div className="mt-2 mb-4 text-sm text-blue-800 dark:text-blue-300">
                {title} Responses for {year}
            </div>
            {
                showDetails && <div className="w-full">
