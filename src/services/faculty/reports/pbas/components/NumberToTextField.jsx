...
const NumberToTextField = ({ facultyTableAvailable, label, activity, state, setState, classes = "", isForm = false, options, model, casYearState, activityName, addName, isFile = true, calculateScore = true, addOnce = false, saveLoader, setSaveLoader, scoreCalculator = false }) => {
    ...
    let param = {
        ...
        dataFilter: model.includes("Book") ? model === 'MainBookAndChapter' ?
            { userId: user?._id, year: casYearState, type: { $in: ['Book', 'Chapter', 'Editor', 'Translator'] } } : { userId: user?._id, year: casYearState, type: 'Conference' } : null
    }
                    ...
                    <div className='flex flex-col items-end justify-end'>
                        <div className='flex items-center justify-end gap-3'>
                            <div className="btn-group" role="group" aria-label="Fetch years">
                                <button type="button" className="btn border-blue-900 border p-2 bg-blue-700 rounded-xl text-white hover:bg-blue-600 duration-200 ease-in-out" onClick={() => { setDataFilterModal({ isOpen: true, year: casYearState }); }}>Fetch {casYearState} Data</button>
                            </div>
